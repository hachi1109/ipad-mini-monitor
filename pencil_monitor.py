#!/usr/bin/env python3
"""Apple 日本 整備済製品(アクセサリ)ストアの「Apple Pencil Pro」在庫監視 → Discord / LINE / メール通知

iPad mini 版(monitor.py)と同じ仕組みです。対象ページと検出条件だけが異なります。

使い方:
  python3 pencil_monitor.py            # 3分おきに常時監視(Mac向け)
  python3 pencil_monitor.py --once     # 1回だけ確認して終了(GitHub Actions向け)
  python3 pencil_monitor.py --test     # テスト通知を送って終了
  python3 pencil_monitor.py --dry-run  # 通知せず、検出結果だけ表示(動作確認用)
...
その他の環境変数:
  ...
  STATE_FILE           (任意, 既定 pencil_seen.json  通知済み商品の記録。他の監視とは別ファイル)
"""
...
STORE_URL = "https://www.apple.com/jp/shop/refurbished/accessories"
BASE_URL = "https://www.apple.com"

# 検出条件(全角/半角・ハイフン種類の違いは NFKC 正規化で吸収した上で判定)
# 「Apple Pencil」かつ「Pro」を両方含む商品名にマッチ(第1/第2世代・USB-C版とは区別する)
TARGET_PATTERNS = [
    re.compile(r"apple\s*pencil", re.I),
...
STATE_FILE = Path(os.environ.get("STATE_FILE", "pencil_seen.json"))
...
def check_once(dry_run: bool = False) -> None:
    ...
    for p in hits:
        if p["id"] in seen:
            continue
        subject = "【整備済製品】Apple Pencil Pro が入荷しました"
        body = f"{p['name']}\n{p['url']}"
        if dry_run:
            log(f"[dry-run] 通知内容:\n{subject}\n{body}")
            continue
...
def main() -> int:
    ...
    if args.test:
        try:
            notify("【テスト】Apple Pencil Pro 整備済製品モニター", "この通知が届けば、設定は正常です。")
            return 0
        except Exception as e:  # noqa: BLE001
            log(f"エラー: {e}")
            return 1
    ...
    while True:
        try:
            ...
            if fails >= FAIL_ALERT_THRESHOLD and not alerted:
                try:
                    notify("【警告】Apple Pencil Pro モニター", f"監視が{fails}回連続で失敗しています: {e}")
                    alerted = True
