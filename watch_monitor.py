#!/usr/bin/env python3
"""Apple 日本 整備済製品(Apple Watch)ストアの「Apple Watch Series 11」在庫監視 → Discord / LINE / メール通知

iPad mini 版(monitor.py)と同じ仕組みです。対象ページと検出条件だけが異なります。

使い方:
  python3 watch_monitor.py            # 3分おきに常時監視(Mac向け)
  python3 watch_monitor.py --once     # 1回だけ確認して終了(GitHub Actions向け)
  python3 watch_monitor.py --test     # テスト通知を送って終了
  python3 watch_monitor.py --dry-run  # 通知せず、検出結果だけ表示(動作確認用)

通知先(monitor.py と共通の環境変数をそのまま使います。1つ以上が必須, --dry-run 以外):
...
その他の環境変数:
  ...
  STATE_FILE           (任意, 既定 watch_seen.json  通知済み商品の記録。monitor.py の seen.json とは別ファイル)
"""
...
STORE_URL = "https://www.apple.com/jp/shop/refurbished/watch"
BASE_URL = "https://www.apple.com"

# 検出条件(全角/半角・ハイフン種類の違いは NFKC 正規化で吸収した上で判定)
# 「Apple Watch」かつ「Series 11」を両方含む商品名にマッチ(サイズ・色・GPS/Cellularは問わない)
TARGET_PATTERNS = [
    re.compile(r"apple\s*watch", re.I),
    re.compile(r"series\s*11\b", re.I),
]
...
STATE_FILE = Path(os.environ.get("STATE_FILE", "watch_seen.json"))
...
def check_once(dry_run: bool = False) -> None:
    ...
    for p in hits:
        if p["id"] in seen:
            continue
        subject = "【整備済製品】Apple Watch Series 11 が入荷しました"
        body = f"{p['name']}\n{p['url']}"
        if dry_run:
            log(f"[dry-run] 通知内容:\n{subject}\n{body}")
            continue
...
def main() -> int:
    ...
    if args.test:
        try:
            notify("【テスト】Apple Watch Series 11 整備済製品モニター", "この通知が届けば、設定は正常です。")
            return 0
        except Exception as e:  # noqa: BLE001
            log(f"エラー: {e}")
            return 1
    ...
    while True:
        try:
            ...
            if fails >= FAIL_ALERT_THRESHOLD and not alerted:
                try:
                    notify("【警告】Apple Watch Series 11 モニター", f"監視が{fails}回連続で失敗しています: {e}")
                    alerted = True
